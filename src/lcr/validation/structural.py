...
_PROTECTED_ABBREVS = re.compile(r'\b(?:et al|Fig|e\.g|i\.e|Dr|Prof|vs|etc)\.')
FORBIDDEN_PATTERNS = [
...
def structural_check(answer: str, max_index: int) -> StructuralCheckResult:
    ...
    # 保护缩写不被误切
    text = _PROTECTED_ABBREVS.sub(lambda m: m.group(0).replace('.', '\x00'), answer)
